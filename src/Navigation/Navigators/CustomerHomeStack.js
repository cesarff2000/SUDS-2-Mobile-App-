...
import NotificationList from '../../CustomerScreen/NotificationList';
...
const CustomerHomeStack = () => {
     ...
     const Noticon = ({ navigation }) => (
          <TouchableOpacity style={{ padding: 2 }} onPress={() => navigation.toggleDrawer()}>
               <Image style={{ height: 20, width: 20, tintColor: 'white' }} source={require('../../../Assets/notibell.png')} />
          </TouchableOpacity>
     );
     return (
          <Stack.Navigator initialRouteName="DASHBOARD" mode="modal" screenOptions={defaultScreenOptions}>
               <Stack.Screen name="DASHBOARD" options={{ headerLeft: () => <Icon navigation={nav.current} />, headerTitle: title, headerRight: () => <Iconto navigation={nav.current} /> }} component={CustomerDrawer} />
               ...
               <Stack.Screen name="Notification List" component={NotificationList} />
          </Stack.Navigator>
     );
};

const Iconto = ({ navigation }) => (
     <TouchableOpacity style={{ padding: 10 }} onPress={() => navigation.navigate('Notification List')}>
          <Image style={{ height: 20, width: 20, tintColor: 'white' }} source={require('../../../Assets/notibell.png')} />
     </TouchableOpacity>
);
