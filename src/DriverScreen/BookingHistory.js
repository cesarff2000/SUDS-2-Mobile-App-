...
import { ACTIONS, BookingContext, WASHER_ACCEPTED, WASHER_ARRIVED, WASHR_ON_THE_WAY, WASH_AUTO_CANCELLED, WASH_CANCELLED, WASH_COMPLETED, WASH_IN_PROGRESS, WASH_PENDING, WASH_REJECTED } from '../Providers/BookingProvider';


const BookingHistory = ({ navigation }) => {
    const netInfo = useNetInfo();
    const { setNewJobRequestId,changeNewJobfun } = useContext(AppContext)
    const {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <FlatList
                ...
                data={bookingHistory}
                renderItem={({ item }) => <Item item={item} navigation={navigation}  pagerefrash={pagerefrash}/>}
...
const Item = ({ item, navigation,pagerefrash }) => {
    ...
    const onPress = async () => {
        switch (item.status) {
            ...
            case WASH_REJECTED: return callrejectfun()
            case WASH_AUTO_CANCELLED: return Alert.alert('Auto Cancellation!', 'This job request has been automatically canceled due to a schedule conflict.')
    ...
    const callrejectfun = () =>{
        Alert.alert('Rejected', 'You rejected this job request')
        pagerefrash();
    }
