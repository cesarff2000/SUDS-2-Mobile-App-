...
const ScheduleList = ({ navigation, route, disabled }) => {
  ...
  useEffect(() => {
    // console.log('useeffect...', currentBooking?.washer_id)
  ...
  // const checkUnavailable = (val) => {
  //   setButtonAEnabled(false);
  //   console.log('washerScheduleData3...', washerScheduleData)
  //   if (unavailabletime) {
  //     if (washerScheduleData.length > 0 && washerScheduleData.includes(val)) {
  //       return '#c43636';
  //     } else {
  //       return 'white';
  //     }
...
const DailySchedule = ({ dismiss, item, date, onDChange, checkBooking }) => {
  ...
  const washerUnavailableSetData = async () => {
    AsyncStorage.getItem("washer_id").then(async (result) => {
      ...
      //   }
      //   else{
      //     Alert(alert("Currently you have an orders on this day"));
