...
const OnJob = ({ navigation, route }) => {
    ...
    const waitfortimfun = async () => {
        Alert.alert('Alert', 'You can`t start the job as booking date is beyond from the current date')
    };
    ...
    const gettimecurrnt = (bookingone) => {
        let currentdate = moment(new Date()).format("YYYY-MM-DD");
        let currenttime = moment(new Date()).format("h:mm:ss");
        let bookdate =  bookingone.booking_date;
        let booktime =  bookingone.booking_time;
    ...
    const openlanchingnavi = (bookingone)=>{
        
        let dlat =Number(bookingone.wash_lat_lng.latitude);
        let dlng =Number(bookingone.wash_lat_lng.longitude); 
        let plat=Number(currentlat);
        let plng =Number(currentlng);
                          
        let destination = [dlat,dlng];
        let options= {
            start:[plat,plng]
        };
        LaunchNavigator.navigate(destination,options).then(success =>{
        ...
        default: return (
            <View style={{ flex: 1, position: 'relative' }}>
                ...
                <View style={{
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', }}>
                        <TouchableOpacity
                            ...
                            style={{ flexDirection: 'row', padding: 10, width: '60%' }}>
                        ...
                        <TouchableOpacity  onPress={() =>openlanchingnavi(booking)} style={{ flexDirection: 'row', padding: 10, width: '40%' }}>
                            <Text style={{ padding: 4, marginLeft: 5, fontSize: 12 }}>NAVIGATION</Text>
                            <Image style={{ width: 30, height: 30, marginLeft:5 }} source={require('./../../Assets/lanch-navi.png')} />
                        </TouchableOpacity>
                    </View>
                    <View style={{ width: '100%', height: 1, backgroundColor: '#aaa' }} />
                ...
                <View style={{ backgroundColor: '#efefef', padding: 10 }}>
                        ...
                        <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <View style={{}}>
                                ...
                                <View style={{ flex: 1, flexDirection: 'row', marginTop: 5 }}>
                                    ...
                                    <Text style={{ marginHorizontal: 3, color: '#999' }}>{moment(booking.booking_date).format("DD-MM-YY")}  {booking.booking_time}</Text>
                                </View>
                            </View>
                            <View style={{}}>
                    ...
                    {
                        (gettimecurrnt(booking)==true)?(
                            <CtaButton
                                onPress={waitfortimfun}
                                primary
                                title={'WAIT FOR BOOKING DATE'}
